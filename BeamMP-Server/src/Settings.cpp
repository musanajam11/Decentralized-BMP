...
// Changes: Added General_BackendUrl setting with configurable default.
// Backend URL is read from ServerConfig.toml or BEAMMP_BACKEND_URL env var.
...
Settings::Settings() {
    SettingsMap = std::unordered_map<Key, SettingsTypeVariant> {
        ...
        { General_BackendUrl, std::string("https://backend.yourdomain.xyz") },
    ...
    InputAccessMapping = std::unordered_map<ComposedKey, SettingsAccessControl> {
        ...
        { { "General", "BackendUrl" }, { General_BackendUrl, READ_WRITE } },
