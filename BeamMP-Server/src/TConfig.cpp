...
//
// MODIFIED by Decentralized-BMP contributors, 2024-2026.
// Changes: Added BackendUrl config key parsing from ServerConfig.toml
// and BEAMMP_BACKEND_URL environment variable.
...
static constexpr std::string_view StrBackendUrl = "BackendUrl";
static constexpr std::string_view EnvStrBackendUrl = "BEAMMP_BACKEND_URL";
...
void TConfig::FlushToFile() {
    ...
    data["General"][StrBackendUrl.data()] = Application::Settings.getAsString(Settings::Key::General_BackendUrl);
    SetComment(data["General"][StrBackendUrl.data()].comments(), " The URL of the backend server to use for authentication, server list, and heartbeat. Change this to point at a different backend.");
...
void TConfig::ParseFromFile(std::string_view name) {
    try {
        ...
        TryReadValue(data, "General", StrBackendUrl, EnvStrBackendUrl, Settings::Key::General_BackendUrl);
        // Misc
