...
This fork adds a **configurable backend URL**. Instead of connecting to the official BeamMP backend, the server reads `BackendUrl` from `ServerConfig.toml` (or the `BEAMMP_BACKEND_URL` environment variable) and directs all authentication, heartbeat, and server-list calls there.

### Configuration

Add `BackendUrl` under `[General]` in your `ServerConfig.toml`:
