/*
 ...
 Changes: Added configurable BackendUrl (read from Launcher.cfg JSON,
 with default fallback). Replaces hardcoded backend.beammp.com endpoint.
*/
...
std::string Branch;
std::string BackendUrl = "https://backend.yourdomain.xyz";
...
void ParseConfig(const nlohmann::json& d) {
    ...
    if (d.contains("BackendUrl") && d["BackendUrl"].is_string()) {
        BackendUrl = d["BackendUrl"].get<std::string>();
        // Strip trailing slash if present
        while (!BackendUrl.empty() && BackendUrl.back() == '/') {
            BackendUrl.pop_back();
        }
