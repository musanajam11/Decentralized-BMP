/*
 ...
 Changes: Launcher/mod update and build download URLs now use the
 configurable BackendUrl instead of hardcoded backend.beammp.com.
*/
...
void CheckForUpdates(const std::string& CV) {
    std::string LatestHash = HTTP::Get(BackendUrl + "/sha/launcher?branch=" + Branch + "&pk=" + PublicKey);
    std::string LatestVersion = HTTP::Get(
        BackendUrl + "/version/launcher?branch=" + Branch + "&pk=" + PublicKey);
    ...
    if (FileHash != LatestHash && IsOutdated(Version(VersionStrToInts(GetVer() + GetPatch())), Version(VersionStrToInts(LatestVersion)))) {
        if (!options.no_update) {
            ...
            if (HTTP::Download(
                BackendUrl + "/builds/launcher?download=true"
                "&pk="
...
void EnableMP() {
    ...
    if (db.is_open()) {
        ...
        if (d.contains("mods")) {
            if (!d["mods"].contains("multiplayerbeammp")) {
                d["mods"]["multiplayerbeammp"] = {
                    {"active", true},
                    {"dirname", "/mods/multiplayer/"},
                    {"filename", "beammp.zip"},
                    {"fullpath", "/mods/multiplayer/beammp.zip"}
                };
            } else {
...
void PreGame(const beammp_fs_string& GamePath) {
    ...
    if (!options.no_download) {
        std::string LatestHash = HTTP::Get(BackendUrl + "/sha/mod?branch=" + Branch + "&pk=" + PublicKey);
        transform(LatestHash.begin(), LatestHash.end(), LatestHash.begin(), ::tolower);
        ...
        if (FileHash != LatestHash) {
            ...
            HTTP::Download(BackendUrl + "/builds/client?download=true"
                           "&pk="
                    + PublicKey + "&branch=" + Branch,
                ZipPath, LatestHash);
        }
