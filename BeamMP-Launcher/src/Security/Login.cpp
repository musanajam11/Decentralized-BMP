/*
 ...
 Changes: User authentication requests now target the configurable
 BackendUrl instead of hardcoded backend.beammp.com.
*/
...
std::string Login(const std::string& fields) {
    ...
    try {
        std::string Buffer = HTTP::Post(BackendUrl + "/userlogin", fields);
...
void CheckLocalKey() {
    if (fs::exists("key") && fs::file_size("key") < 100) {
        ...
        if (Key.is_open()) {
            ...
            Buffer = HTTP::Post(BackendUrl + "/userlogin", R"({"pk":")" + Buffer + "\"}");
